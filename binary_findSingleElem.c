/*
 * Find the element that appears in an array only once where all other
 * elemts appear twice
 * Find the element that appears in an array only once where all other
 * elemts appear thrice
 */

 #include <stdio.h>

 void findSingular_in2repeats(int* array, int len)
 {
   int res=0, idx;
   ...
   printf("\n Singular element in 2 rep array = %d. \n", res);
 }

 /*
  * Algorithm:
  * If a number came once, put it to "ones", if it came twice, remove from
  * "ones" and put it to "twos". If it came thrice, remove it from both "ones"
  * and "twos"
  */
  void findSingular_in3repeats(int* array, int len)
  {
    int id=0, ones=0, twos=0, thrice_occured = 0, x=0;
    for(id=0; id<len; id++)
    {
      x = array[id]; /* Get the array element */
      /* if the element has been in ones, put it to twos, if it is not
       * in ones, do nothing */
      twos |= (ones & x);
      /* if the element is in ones already, remove it, else add it to ones */
      ones ^= x;
      /* if the element is in both ones and twos, remove it from both */
      thrice_occured = ~(ones & twos);
      twos &= thrice_occured;
      ones &= thrice_occured;
    }
    printf("\n Singular element in 3 rep array = %d. \n", ones);
  }

 int main()
 {
   //aray where all other elemets except 5 appear thrice
   int array_2reps[] = {9,2,1,3,2,1,4,7,9,5,3,4,7};
   int len_2reps = sizeof(array_2reps)/sizeof(array_2reps[0]);

   //aray where all other elemets except 7 appear thrice
   //int array_3reps[] = {1,2,3,5,9,2,4,1,3,2,1,4,7,5,9,5,9,3,4};
   int array_3reps[] = {1,2,3,1,1,2,2};
   int len_3reps = sizeof(array_3reps)/sizeof(array_3reps[0]);

  findSingular_in2repeats(array_2reps, len_2reps);
  findSingular_in3repeats(array_3reps, len_3reps);
 }

 /*
 DETAILED EXPLANATION for the second part:
